...
let parse_arguments argv f msg =
  try
    ...
    Arg.parse_and_expand_argv_dynamic current (ref argv) arg_spec f msg
  with
...
let reset_flags () =
  ...
  no_check_prims := false;
  bytecode_compatible_32 := false;
  output_c_object := false;
  output_complete_object := false;
  all_ccopts := [];
  classic := false;
  nopervasives := false;
  match_context_rows := 32;
  preprocessor := None;
  all_ppx := [];
  absname := false;
  annotations := false;
  ...
  dump_clambda := false;
  dump_rawflambda := false;
  dump_flambda := false;
  dump_flambda_let := None;
  dump_flambda_verbose := false;
  dump_instr := false;
  keep_camlprimc_file := false;
  keep_asm_file := false;
  ...
  dump_linear := false;
  dump_interval := false;
  keep_startup_file := false;
  dump_combine := false;
  profile_columns := [];
  debug_runavail := false;
  native_code := false;
  force_slash := false;
  clambda_checks := false;
  flambda_invariant_checks := Config.with_flambda_invariants;
  dont_write_files := false;
  shared := false;
  dlcode := true;
  pic_code := (Config.architecture == "amd64");
  runtime_variant := "";
  keep_docs := false;
  keep_locs := true;
  unsafe_string := (if Config.safe_string then false else not Config.default_safe_string);
  classic_inlining := false;
  inlining_report := false;
  afl_instrument := Config.afl_instrument;
  afl_inst_ratio := 100;
  simplify_rounds := None;
  default_simplify_rounds := 1;
  inline_threshold := (Float_arg_helper.default default_inline_threshold);
  inline_toplevel_threshold := (Int_arg_helper.default default_inline_toplevel_threshold);
  inline_call_cost := (Int_arg_helper.default default_inline_call_cost);
  inline_alloc_cost := (Int_arg_helper.default default_inline_alloc_cost);
  inline_prim_cost := (Int_arg_helper.default default_inline_prim_cost);
  inline_branch_cost := (Int_arg_helper.default default_inline_branch_cost);
  inline_indirect_cost := (Int_arg_helper.default default_inline_indirect_cost);
  inline_branch_factor := (Float_arg_helper.default default_inline_branch_factor);
  inline_lifting_benefit := (Int_arg_helper.default default_inline_lifting_benefit);
  inline_max_unroll := (Int_arg_helper.default default_inline_max_unroll);
  inline_max_depth := (Int_arg_helper.default default_inline_max_depth);
  unbox_specialised_args := true;
  unbox_free_vars_of_closures := true;
  unbox_closures := false;
  unbox_closures_factor := default_unbox_closures_factor;
  remove_unused_arguments := false
